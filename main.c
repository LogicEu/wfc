#define WAVEFUNCOLLAPSE_APPLICATION
#include "wfc.h"
...
static void wfc_print_help(const char* name)
{
    ...
    printf("-p\t\t: set periodic flag to true, output image warp edges\n");
    ...
    printf("-H <arg>\t: how to select pixel to observe next (0 = entropy, 1 = mrv, 2 = scanline)\n");
}

static void wfc_model_flush(const struct wfc_model* model, bmp_t* bmp, Px* fb, bool periodic)
{
...
int main(const int argc, const char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (argv[i][0] == '-' && !argv[i][2]) {
            ...
            switch (argv[i][1]) {
                ...
                default: {
                    fprintf(stderr, "Unrecognized option %s. See -help for more information\n", argv[i]);
                    return EXIT_FAILURE;
                }
            }
            if (n) {
                if (i + 1 == argc) {
                    fprintf(stderr, "Missing argument for option %s. See -help for more information\n", argv[i]);
                    return EXIT_FAILURE;
                }
                *n = atoi(argv[++i]);
            }
    ...
    struct wfc_model model = wfc_model_create(bmp.pixels, bmp.width, bmp.height, width, height, N, periodic, symmetry);
